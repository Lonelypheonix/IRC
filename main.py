...
SAVE_DIRECTORY = r"D:\NYCU\Semester 2\RNN and LSTM\final project\VB"
...
def get_vector_store() -> InMemoryVectorStore:
    """全域唯一的向量庫，存在 session_state 裡，避免每次 rerun 重建。"""
...
def extract_images_with_pymupdf(pdf_path: str,
        ...
        for img_i, info in enumerate(page.get_images(full=True)):
            ...
            if have_ollama:
                try:
                    ...
                    sidebar.warning(f"Vision 描述失敗: {e}")
    ...
    doc.close()
    sidebar.success(f"✅ PyMuPDF 共匯出 {len(results)} 張 PNG")
    return results
...
def _check_ollama(sidebar) -> bool:
    try:
        ...
        sidebar.warning("⚠️ 找不到本機 Ollama；Vision 描述將跳過")
        return False
...
def extract_references(full_text: str) -> List[str]:
    ...
    return cleaned_titles[:]  # Return up to 30 references
